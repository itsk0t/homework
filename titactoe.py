...
class TicTacToe:
    ...
    def move(self):  # ход
        def is_move_valid(x, y):
            return 1 <= x <= 3 and 1 <= y <= 3 and self.table[x-1][y-1] == '*'  # проверка введенных координат

        print(f'Ход игрока {self.get_current_player_name()}:')
        is_coords_valid = False  # что-то типа переменной для проверки верны ли координаты
        x, y = 0, 0
        while not is_coords_valid:  # цикл с проверкой 
        ...
        self.table[x-1][y-1] = self.current_move  # сам ход 
    ...
    def get_winner(self):  # поиск победителя
        ...
        for line in self.table:
            if line[0] != '*' and len(set(line)) == 1:  # проверка по вертикали и горизонтали
                self.winner = self.current_move
                return None
            if '*' in line:  # проверка на ничью
                count_stars += 1
        ...
        for i in range(3):
            if '*' != self.table[0][i] == self.table[1][i] == self.table[2][i]:
                self.winner = self.current_move
                return None
        if '*' != self.table[0][0] == self.table[1][1] == self.table[2][2] and '*' != self.table[0][2] == self.table[1][1] == self.table[2][0]:  # проверка по диагонали 
            self.winner = self.current_move
            return None
